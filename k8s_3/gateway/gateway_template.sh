...
virt-customize -a debian-12-generic-amd64.qcow2 --install qemu-guest-agent,wget,ssh,vim,tmux,curl,ntp,iptables-persistent
# virt-customize -a debian-12-generic-amd64.qcow2 --run-command 'apt-get install ssh vim tmux curl ntp iptables-persistent -y'
virt-customize -a debian-12-generic-amd64.qcow2 --run-command 'systemctl enable ntp; systemctl start ntp; systemctl enable ssh; systemctl start ssh'
